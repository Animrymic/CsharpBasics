...
public class DataAccess
{
    ...
    {
        Trainer? trainerFromDb = _db.Trainers
            .FirstOrDefault(x => x.Username == username && x.Password == password);
    ...
    {
        Student? studentFromDb = _db.Students
            .FirstOrDefault(x => x.Username == username && x.Password == password);
    ...
    public List<string> GetUsernames(Role role, Admin loggedAdmin)
    {
        ...
        {
            case Role.Admin:
                return _db.Admins.Where(x => x.Username != loggedAdmin.Username)
                                 .Select(x => x.Username)
                                 .ToList();
            case Role.Trainer:
            ...
            default:
                throw new Exception("Error occuered while retrieveing usernames from database");
        }
    }
